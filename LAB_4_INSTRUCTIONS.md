...
For this part, use an array of structs with the above menu names and pointers to appropriate functions that implement each option, using the same menu scheme from lab 1.
...
**Toggle debug mode** means turn the debug flag on (if it is currently off, which it is in the initial state), and print (to stderr) "Debug flag now on". If the debug flag is on, this function prints "Debug flag now off", and turns the flag off. When the debug mode is on, you should print the value of the variables: `unit_size`, `file_name`, and `mem_count`, every time just before the menu is printed.
...
**Set File Name** queries the user for a file name, and stores it in `file_name`. You may assume that the file name is no longer than 100 characters. If debug mode is on, the function should also print (to stderr, as are all debug messages): "Debug: file name set to 'file_name' " (obviously, replacing 'file_name' with the actual name).
...
4. If not valid, print an error message and leave unit_size unchanged.
...
Later on, in task 1, you are required to write code that handles data in unit sizes (i.e. not necessarily single bytes). This might confuse you into writing much more code than needed. See below how to handle multiple unit sizes when reading, printing etc. without writing too much code. Download and extract the files in `units.zip` from the Lab 4 Auxiliary Files folde. Examine the code we provide there. Make sure you look at this code and understand how to use it before attending the lab. As a minimum, temporarily make the display-memory stub print a constant number (e.g. 0x5555) using the current size, in decimal and hexadecimal, as an exercise. Relevant to this task is the function `print_units`, and also see below.

Note that, you can use the following lines for printing:
...
1. Check if `file_name` is empty (i.e. equals to ""), and if it is print an error message and return.
2. Open `file_name` for reading. If this fails, print an error message and return.
3. Prompt the user for location in hexadecimal, and length (in decimal).
4. If debug flag is on, print the `file_name`, as well as location, and length to stderr.
5. Copy `length * unit_size` bytes from `file_name` starting at position `location` into `mem_buf`.
6. Close the file.

Suppose that the user has already set the file name to "abc". If users choose 3 on the menu, they are prompted for location and length. It should look as follows (exact output format is not critical):
...
**Remember:**
- To read location (hexadecimal) and length (decimal) use `fgets` and then `sscanf`, rather than `scanf` directly.
...
Toggle display mode means turn the decimal display flag on and print using a decimal representation, the initial state is off and print using a hexadecimal representation. Print "Decimal display flag now on, decimal representation", and if the display flag is on, this function prints "Decimal display flag now off, hexadecimal representation", and turns the flag off. For exmple, (assume that the display flag is off now) entering option 4 in the menu:

```
4
Decimal display flag now on, decimal representation
4
Decimal display flag now off, hexadecimal representation
```

#### Task 1c: Memory Display

Write the function for "Memory Display" option:

This option displays `u` units of size `unit_size` starting at a given address `addr` in memory. `Unit_size` is already defined in the global variables, but `u` and `addr` should be queried from the user by these functions. `u` will be given in decimal and offset (resp. `addr`) in hexadecimal. Entering a value of 0 for `addr` is a special case, in which the memory to be displayed starts at the beginning of your `mem_buf`.

The units should be displayed according to the decimal display flag. If the display flag is on then print using a decimal representation, and if the decimal display flag is off print using a hexadecimal representation.

As an example of using Memory Display, if the user set the unit size to 2 and loaded a file into memory, then the output should look something like this (remember, we previously loaded 5 pairs of bytes from the file abc, located 303-312):
...
**Use your newly implemented functionality (and the ELF manual) to answer: what is the entry point of your own hexeditplus program? Verify your answer using `readelf -h`**
...
- `source-address` (source virtual memory address, in hexadecimal), `source-address` can be set to 0: this is a special case, where the source address is start of `mem_buf`, in any other case, use `source-address` as an address in (virtual) memory.
...
3. Replace a unit at `location` in the memory with the value given by `val`.
...
1. Run the file.
2. Which "function" should precede `main( )` in execution? (Hint: see assembly code in Lab 3).
...
Use `hexeditplus` to replace (patch) the buggy `count_digits` function in the `ntsc` file with the corrected version from the new program.

You should do it using options 3 & 6 in `hexeditplus`.

(think: are there any kinds of restrictions on the code you wrote for the `count_digits` function?)
...
All tasks must be completed during the regular lab. The deliverables must be submitted until the end of the lab session.
